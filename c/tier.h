...
#define TIER_WIN_OPS      256u  /* probes per hit-rate learning window */
 ...
 * reuse the expert had proven. Ghost memory is bounded by census: past
 * ghost_cap the least-recently-routed ghost (which may be the one just
 * created) is forgotten entirely. */
static void tier_evict(TierAdapt *a, int8_t *freq, int nexpert,
    ...
    for(int e=0;e<nexpert;e++) if(freq[e]<0){
        ng++;
...
/* Two feedback loops, checked every TIER_ADAPT_EVERY evictions: the
 * graduation rate drives k, and the hit-rate gradient tunes the rate
 * thresholds so the policy adapts to the workload instead of fixed cutoffs.
 ...
 * delta and re-triggers — a short burst of adapts that lets k track a
 * workload shift in a few steps instead of one step per interval. It reads
 * like a bug; it is not. Don't "fix" it. */
static void tier_maybe_adapt(TierAdapt *a){
    ...
    if(total>100){ a->ev_unprot/=2; a->ev_prot/=2; }  /* last_check kept: arms the wrap burst */
}
