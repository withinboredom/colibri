...
| `PILOT_REAL` | `0` (off) | Value-preserving real cross-layer prefetch loads (`PILOT_REAL=1` opts in). On x86-64 CPU execution, the resolved expert loop primes only the first four cache lines of each next-expert weight matrix with a T2/L3 hint; it does not pull whole expert slabs into cache. |
